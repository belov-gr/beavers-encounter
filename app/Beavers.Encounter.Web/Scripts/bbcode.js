...
var imgstart = -1;      // beginning of the IMG if zero or greater (ignored if -1)
...
function textToHtmlCB(mstr, m1, m2, m3, m4, offset, string)
{
   ...
   if(isValidTag(m2)) {
      ...
      if(opentags.length && (
        (opentags[opentags.length-1].bbtag == "url" && urlstart >= 0) || 
        (opentags[opentags.length-1].bbtag == "img" && imgstart >= 0)))
         return "[" + m2 + "]";
   ...
   if(isValidTag(m4)) {
      ...
      if(m4 == "url") {
         ...
         // if there was no option, use the content of the [img] tag
         if(imgstart > 0)
            return "\">" + string.substr(imgstart, offset-imgstart) + opentags.pop().etag;
...
function parseBBCode(post)
{
   ...
   if(opentags.length) {
      ...
      if(opentags[opentags.length-1].bbtag == "img") {
         opentags.pop();
         endtags += "\">" + post.substr(imgstart, post.length-imgstart) + "</img>";
      }
